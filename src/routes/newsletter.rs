...
pub enum PublishError {
    ...
    #[error(transparent)]
    UnexpextedError(#[from] anyhow::Error),
}
...
impl ResponseError for PublishError {
    fn error_response(&self) -> HttpResponse {
       match self {
           PublishError::UnexpextedError(_) => {
...
async fn validate_credentials(
    ...
    .map_err(PublishError::UnexpextedError)?;
        ...
        match subsciber {
            Ok(subsciber) => {
                email_client
                    ...
                    .await
                    .with_context(|| {
                        format!("Failed to send newsletter issue to {}", subsciber.email)
                    })?;
            }
            Err(error) => {
                tracing::warn!(
                    error.cause_chain = ?error,
                    "Skipping a confirmed subsciber. \
